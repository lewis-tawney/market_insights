...
- **`dist/`** - Built/compiled frontend files (generated by `pnpm run build` or `npm run build`)
  ...
  - **Note**: Development docs and scripts use `npm` (`npm install`, `npm run dev`, `npm run build`); a `pnpm-lock.yaml` is present from earlier tooling but `npm` is the supported default.
...
- **`import_massive_daily.py`** - Backfill Massive daily data into DuckDB
